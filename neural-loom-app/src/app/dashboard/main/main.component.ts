...
export class MainComponent implements OnInit {
  ...
  uploadToFirebase() {
    // let imageFile = this.uploadedImageURL;

    // let temp = 'IMG_' + (Math.round(Math.random()) * 100000).toString();
    // const filename = `${new Date().getTime()}_${temp}`;
    
    // this.storage.ref('images/'+this.afAuth.auth.currentUser.uid).child(filename).putString(imageFile).then(snap => {
    //   if(snap.state == "success") {
    //     this.afdb.list('/users/'+ this.afAuth.auth.currentUser.uid + '/images').push(filename);
    //   }
    // });

    let imageFile = this.uploadedImageURL.toString();
    
    if(imageFile.length == 0) {
      this.flashMessagesService.show("No image available",{cssClass: 'custom-danger-alert' , timeOut:7000});
    } else {
      this.afdb.list('/users/'+ this.afAuth.auth.currentUser.uid + '/images').push(imageFile).then(val => {
        if(val !== undefined) {
          this.flashMessagesService.show("Upload successful",{cssClass: 'custom-success-alert' , timeOut:7000});
        } else {
          this.flashMessagesService.show("Upload failed",{cssClass: 'custom-danger-alert' , timeOut:7000});
        }
      });
