...
def main(args):
    ...
    if load == False:
        model.load_weights(output+'_train_weights.h5',by_name=False)
    ...
    for x in datagen.flow_from_directory(train_image_path, class_mode=None, batch_size=train_batchsize,
        ...
        if i % 500 == 0:
            print("epoc: ", i)
            val_x = net.predict(x)
            net.save_weights(gen_model_name+".h5")
            display_img(i, x[0], style)
            display_img(i, val_x[0],style, True)
            model.save_weights(output+'_train_weights.h5')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gen_model_name' ,type = str ,required = True)
    parser.add_argument('--load' , type = bool , required = True)
